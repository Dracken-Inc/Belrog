...
function getRuntimeCandidates({ userDataPath, comfyRootPath = '', explicitPythonPath = '' }) {
  ...
  const candidates = [
    { pythonPath: managed.pythonPath, kind: 'managed', label: 'Belrog RTX runtime' },
  ]
...
async function checkRtxRuntime(options = {}) {
  ...
  return {
    ...
    error: failures.length > 0
      ...
      : 'Install the optional Belrog RTX runtime to enable direct 4K upscaling. ComfyUI is not required.',
  }
}
...
function downloadFile(urlText, outputPath, options = {}, redirectsRemaining = 5) {
  ...
  return new Promise((resolve, reject) => {
    ...
    signal?.addEventListener?.('abort', abort, { once: true })
    request = transport.get(url, { headers: { 'User-Agent': 'Belrog RTX Runtime Installer' } }, (response) => {
...
async function runRtxVideoUpscale(options = {}) {
  ...
  if (!helperPath || !fs.existsSync(helperPath)) throw new Error('The Belrog RTX helper is missing.')
  if (!ffmpegPath || !fs.existsSync(ffmpegPath) || !ffprobePath || !fs.existsSync(ffprobePath)) {
    throw new Error('Belrog could not find its FFmpeg tools.')
  }
