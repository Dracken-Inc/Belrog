...
const RIFE_SECURE_BUILD_MARKER = 'Belrog secure build: PNG input and output only; WebP is disabled.'
...
const RIFE_WORK_MARKER = '.Belrog-rife-'
...
async function validateReadableFile(filePath, label, {
  ...
  try {
    ...
    throw makeRifeError(
      `${label} could not be found or opened${executable ? ' as an executable' : ''}. Reinstall Belrog to restore Optical Flow.`,
...
async function validateRifeRuntime({
  ...
  try {
    ...
    throw makeRifeError(
      'The portable RIFE model folder is missing or unreadable. Reinstall Belrog to restore Optical Flow.',
      'OPTICAL_FLOW_UNAVAILABLE',
...
async function probeRifeRuntime({
  ...
  try {
    ...
    if (requireSecureBuild) {
      throw makeRifeError(
        `Belrog's trusted smooth-motion engine failed its secure startup check: ${error?.message || String(error)}`,
        'OPTICAL_FLOW_UNAVAILABLE'
      )
    }
  ...
  if (requireSecureBuild && (!compatible || !output.includes(RIFE_SECURE_BUILD_MARKER))) {
    throw makeRifeError(
      'Belrog\'s trusted smooth-motion engine did not report the required PNG-only secure-build marker.',
      'OPTICAL_FLOW_UNAVAILABLE'
    )
  }
  return {
...
async function createOwnedWorkDirectory({ paths, allowedOutputRoot, fsPromises = fs.promises } = {}) {
  ...
  try {
    ...
    if (error?.code === 'EEXIST') {
      throw makeRifeError(
        'A RIFE scratch folder for this job already exists. Wait for that job to finish or restart Belrog.',
        'OPTICAL_FLOW_BUSY'
      )
    }
    throw makeRifeError(
      `Belrog could not create a safe RIFE scratch folder: ${error?.message || String(error)}`,
      'OPTICAL_FLOW_INVALID_INPUT'
    )
  }
}
