"""
Belrog Bridge for ComfyUI.

This package installs a small ComfyUI frontend extension. It does not add
generation nodes; it only adds a "Send to Belrog" action to the ComfyUI
interface so the current graph can be exported as API JSON and posted back to
the embedded Belrog app. (Package folder keeps the legacy comfystudio_bridge
