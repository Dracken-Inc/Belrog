...
belrog v0.3.31 adds animated GIF round trips, a built-in place to discover community work and tutorials, and faster timeline navigation with configurable zoom shortcuts.

## New

- **Animated GIF import.** Import an animated GIF from the Assets panel, drag and drop, MCP, or supported generation results and use it as editable timeline media. belrog preserves the animation timing and creates project-owned playback media for reliable scrubbing, trimming, compositing, and export. Static GIFs remain regular image assets.
- **Optimized GIF export.** Export the timeline or selected range as a continuously looping GIF with adjustable resolution and frame rate. belrog uses an optimized 256-color palette for better quality and more practical file sizes.
- **Discover belrog creations and tutorials.** The new Discover resource brings together Featured videos, work made by the belrog community, and practical tutorials. Videos use click-to-load YouTube playback so the player is not loaded until you choose to watch.
...
- **Resilient Discover catalog.** belrog can refresh the curated catalog independently while retaining a validated bundled fallback for offline or unavailable-network situations.
...
- **Expanded showcase.** Discover now includes eight curated videos, including two additional AI music videos made with belrog.
