# belrog Cloud (Lite) — Design Notes
...
A second build of belrog — tentatively **belrog Cloud** (or
...
- **Endpoints that map directly to what belrog already calls:**

  | belrog needs | Comfy Cloud endpoint |
  | --- | --- |
...
- A **ComfyUI tab** that's a `BrowserView` pointed at
  ...
  User opens/edits custom workflows in-situ. belrog still captures their
...
- **Name candidates:** belrog Cloud, belrog Lite, belrog Go.
- **Tagline direction:** "Runs on any laptop. Pay per generation. Zero setup."
- **Distinct from full belrog:** different icon, different onboarding,
...
- **Migration path:** a user who outgrows Lite (wants custom models, local
  LoRAs, offline use) can switch to full belrog with the same project
...
any of this. The current release cycle stays focused on full belrog
