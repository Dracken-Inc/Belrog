...
- Added FCPXML timeline export from the Export panel, so belrog edits can be imported into DaVinci Resolve, Final Cut Pro, and Premiere workflows for finishing.
...
- Export is currently one-way only: belrog can export `.fcpxml`, but importing XML back into belrog is not included yet.
- Exported XML includes media paths, clip timing, source trims, lanes, and belrog clip metadata notes.
