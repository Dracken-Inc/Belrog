...
belrog v0.3.27 adds PNG image sequence export for compositing, VFX, archival, and frame-by-frame delivery workflows.
...
- **Predictable portable filenames.** belrog creates names such as `project_000001.png`, using a filename stem that is safe across Windows, macOS, and Linux.
...
- **Non-destructive output folders.** belrog creates a new `<name>_png` child folder and refuses to reuse an existing directory, preventing accidental overwrites or unsafe cleanup.
- **Safer cancellation and failure handling.** Incomplete sequence folders are removed when possible. If cleanup cannot complete, belrog reports the exact folder that still contains partial files.
...
- Choose a parent destination and belrog will create a new sequence folder automatically. Existing directories are never claimed as PNG sequence outputs.
