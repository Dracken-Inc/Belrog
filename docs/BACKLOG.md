# Belrog — Come-Back-To-This Backlog
...
## 3. MCP support roadmap from live belrog editing sessions
...
- Added `set_comfyui_connection` so an MCP client can preview or apply a
  local port change in belrog settings.
...
- The setter changes belrog settings only. It does not restart ComfyUI
...
- Added `control_comfyui_launcher` for preview/apply start, stop, and restart
  through the existing belrog launcher. It defaults to preview-only and
...
- Added `list_belrog_workflows` so MCP clients can discover bundled
  workflow ids, categories, local/cloud runtime, image requirements, and source
  files without opening the Generate tab.
- Added `inspect_belrog_workflow` so MCP clients can inspect a bundled or
...
- Added `queue_prepared_generation` so MCP clients can inspect the staged
  ...
  as the normal belrog Queue button. It defaults to preview-only and
...
1. **Launcher port alignment.** Extend repair to notice launcher `extraArgs`
   such as `--port 8190` when belrog is set to `8188`, then propose
   either changing belrog or changing launcher args. This should be
