...
- Adds custom ComfyUI workflow support so advanced users can bring their own API workflow JSONs into belrog.
- Adds a belrog Bridge for the embedded ComfyUI tab, including a `Send to belrog` button for compatible workflows.
...
- Adds endpoint nodes such as `belrog_INPUT_IMAGE`, `belrog_PROMPT`, `belrog_OUTPUT_IMAGE`, `belrog_OUTPUT_VIDEO`, `belrog_WIDTH`, `belrog_HEIGHT`, `belrog_FPS`, `belrog_DURATION`, and `belrog_AUDIO`.
...
- Trims custom music-video audio per shot before sending it to `belrog_AUDIO`.
...
- belrog still depends on a separate local ComfyUI installation.
