...
belrog includes a local Model Context Protocol (MCP) server so AI agents can inspect and operate on the open belrog project. It is designed for agents such as Codex, Claude Code, Cursor-compatible MCP clients, and other open source MCP clients that can talk to a local HTTP MCP server.
...
1. Launch the belrog desktop app.
...
```bash
codex mcp add belrog --url http://127.0.0.1:19790/mcp
```

For Claude Code:

```bash
claude mcp add --transport http belrog http://127.0.0.1:19790/mcp
```
...
belrog keeps this same local config in the repository root for development.

## What Agents Can Do

belrog MCP is useful for five broad workflows:
...
The MCP server is not a replacement for the belrog UI. It is a project-aware control layer for agents. The best results come from asking the agent to inspect first, show a preview plan, then apply only after approval.
...
Do not proxy or expose this port to a network. Any local process that can connect to the port can call the MCP server while belrog is running.
...
6. Use `run_mcp_action_plan` for approved multi-step work so belrog can checkpoint first and stop on the first error.

Undoable timeline changes use belrog's normal undo stack. Project creation, project duplication, exports, generated assets, and imported media can write files to disk.
...
```text
You are connected to belrog. Call get_mcp_recipes, summarize what review and edit passes are available, then inspect the open project with get_project and get_timeline. Do not make changes yet.
```

For a timeline health pass:

```text
Review this belrog timeline for delivery risks. Use analyze_timeline and check_media_health first. If you want to add markers, show me the add_timeline_markers previewOnly plan before applying anything.
```
...
belrog currently exposes 125 MCP tools.
...
| `get_mcp_recipes` | Alias-style recipe entry point for agents asking what belrog MCP can do. |
...
| Tool | Purpose |
| --- | --- |
| `guide_comfyui_setup` | Beginner-friendly setup wizard for connecting belrog to ComfyUI. |
| `diagnose_comfyui_connection` | Diagnose configured localhost port, API health, launcher state, and likely install mode. |
| `set_comfyui_connection` | Preview or set belrog's local ComfyUI port. |
| `repair_comfyui_connection` | Probe likely ports and preview/apply a safe port-setting repair. |
| `control_comfyui_launcher` | Preview/apply start, stop, or restart through belrog's launcher. |
...
| `list_belrog_workflows` | List bundled workflows on the machine. |
| `inspect_belrog_workflow` | Inspect workflow JSON, extract required classes, and validate nodes. |
...
| Tool | Purpose |
| --- | --- |
| `undo` | Undo latest belrog timeline or project-structure edit. |
| `redo` | Redo latest belrog timeline or project-structure edit. |
...
| Tool | Purpose |
| --- | --- |
| `search_stock_media` | Search Pexels photos/videos and open the same results in belrog's Stock tab. |
...
| Tool | Purpose |
| --- | --- |
| `export_timeline` | Preview/start a timeline export through belrog's export worker. |
...
- Make sure the belrog desktop app is running.
...
- Restart belrog after changing development branches or rebuilding Electron code.

### The agent says no project is open

Open a project in belrog, then try again. The agent can call `list_recent_projects` and `open_project`, but most timeline and asset tools need an active project snapshot.
...
- Keep the MCP client connected to the local machine running belrog. This is not a cloud API.
