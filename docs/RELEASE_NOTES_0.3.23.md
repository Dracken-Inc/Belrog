...
Projects moved from Windows to macOS can export normally again. belrog now keeps relocated media paths portable instead of accidentally combining the Mac project folder with an already-absolute path and reporting every source as undecodable.
...
- **Export path resolution is defensive.** belrog recognizes absolute media paths instead of joining them to the project folder a second time.
