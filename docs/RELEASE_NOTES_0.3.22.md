...
Generation-heavy edits no longer need a long browser-clicking session: MiniMax H3 reference-and-audio jobs can be queued, monitored, and returned to the edit through belrog's MCP bridge. Large alternate-take timelines also gain video-track solo for fast review.

## Highlights

- **Direct MiniMax H3 generation over MCP.** Queue a paid H3 reference-to-video performance from an exact belrog image asset and audio segment, choose 768P or 2K output, preserve the requested aspect ratio and seed, and identify the shot without opening the ComfyUI canvas.
...
- Registers the MiniMax H3 reference workflow in belrog's workflow catalog and generic prompt-batch routing.
