...
[![Download for Windows](https://img.shields.io/badge/Windows-下载-0078D4?style=for-the-badge)](https://github.com/belrogLabs/belrog/releases/latest)
[![Download for macOS](https://img.shields.io/badge/macOS-下载-1a1a1a?style=for-the-badge&logo=apple&logoColor=white)](https://github.com/belrogLabs/belrog/releases/latest)
[![Download for Linux](https://img.shields.io/badge/Linux-下载-E95420?style=for-the-badge&logo=linux&logoColor=white)](https://github.com/belrogLabs/belrog/releases/latest)
...
<p align="center"><img src="../readme/agent-editing.gif" alt="一句提示词：Claude 通过 MCP 在 belrog 中完成剪辑" width="860"></p>
<p align="center"><i>一句提示词。智能体生成素材、搭建时间线并混音 — 全程实时，通过 MCP 完成。</i></p>

> 本翻译尽力保持更新。如有不清楚之处，请以[英文版 README](../../README.md) 为准。欢迎提交 PR 改进翻译！

belrog 是一款面向 ComfyUI 用户的开源桌面 AI 视频工作站。它将策划、生成、素材管理、时间线剪辑、字幕、特效和导出整合到一个以项目为单位的应用中。

你可以使用内置的本地和云端工作流，导入自己的 ComfyUI API 工作流 JSON，或安装随附的 belrog Bridge，让在 ComfyUI 中打开的图直接发送回 belrog。

<p align="center">
  <img src="../readme/editor-timeline.png" alt="belrog 编辑器：生成的素材、预览、时间线轨道和检查器" />
</p>
...
belrog 不是 ComfyUI 的替代品。它是 ComfyUI 之上的制作层：规划工作、把任务发送给 ComfyUI、收集输出，并完成最终剪辑。

<p align="center">
  <img src="../readme/create-workflows.png" alt="belrog Create 工作区：UGC、商业广告、音乐视频和短片创作器" />
</p>

## 下载

大多数用户应从 [GitHub Releases 页面](https://github.com/belrogLabs/belrog/releases)下载打包好的桌面应用。
...
除非你打算从源码构建 belrog，否则请忽略 GitHub 自动生成的源码压缩包。
...
- Custom Image 和 Custom Video 工作流，供希望用 belrog 运行自己 ComfyUI API 图的用户使用。
- API JSON 导入，供偏好从 ComfyUI 手动导出工作流的高级用户使用。
- belrog Bridge 支持：兼容的图可以从 ComfyUI 发送到 belrog 的对应面板。
- 工作流环境检查：检测缺失的节点、模型、凭据和配置。
- Featured / My Workflows / Templates 浏览器，带 Local 和 Cloud 筛选。导入的社区工作流会出现在 Featured 中，与内置工作流并列。

<p align="center">
  <img src="../readme/generate-featured.png" alt="belrog Generate 浏览器：精选工作流、本地和云端筛选以及依赖检查器" />
</p>

Templates 标签页可浏览官方 ComfyUI 模板目录（500 多个模板，含大小和热度信息），并将任意模板在内嵌的 ComfyUI 标签页中打开。

<p align="center">
  <img src="../readme/generate-templates.png" alt="belrog 模板浏览器：官方 ComfyUI 模板目录，含分类和筛选" />
</p>

### Create（创作）

Create 包含基于 belrog Director Mode 引擎构建的引导式创作工作流。
...
<p align="center">
  <img src="../readme/export-settings.png" alt="belrog 导出设置：预设、编解码器控制和导出队列" />
</p>
...
<p align="center">
  <img src="../readme/stock-pexels.png" alt="belrog Stock 标签页：Pexels 照片和视频搜索" />
</p>

### ComfyUI 集成

belrog 与本地 ComfyUI 服务器通信，也可以帮助启动它。
...
belrog 内置本地 MCP 服务器，提供 100 多个工具，支持 Codex、Claude Code、兼容 Cursor 的工具及其他 MCP 客户端。
...
写入类工具会先预览执行计划，经批准后才应用，且全部走 belrog 的常规撤销栈。MCP 是智能体辅助审阅、时间线操作、图形润色和生成工作流的推荐自动化途径。

<p align="center">
  <img src="../readme/agents-mcp.png" alt="belrog Agents (MCP) 设置：运行中的本地服务器、连接命令和完整工具列表" />
</p>

## 自定义工作流

自定义工作流是 belrog 存在的主要原因之一。
...
2. 在 ComfyUI 中修改它。
3. 保留所需的 belrog 端点节点。
4. 用 belrog Bridge 发送回来，或手动导入 API 工作流 JSON。
5. 在 belrog 中将该图作为创作流程的一部分或从 Generate 运行。
...
推荐使用精确的 `belrog_*` 标题，但 belrog 也能识别诸如 `belrog input image` 这样的可读标题。仍在使用 `COMFYSTUDIO_*` 标记标题的旧图出于向后兼容依然受支持。

如果某个端点存在，belrog 就可以注入对应的值；如果不存在，则由图自行控制该设置。

<p align="center">
  <img src="../readme/comfyui-bridge.png" alt="内嵌 ComfyUI 图：belrog 端点节点和 Send to belrog 按钮" />
</p>
