...
## Where to enter it in belrog

Open:

`Settings > ComfyUI Connection > Comfy Account API Key`

belrog sends this key as `extra_data.api_key_comfy_org` when queueing compatible workflows.
...
- If a cloud workflow loads in ComfyUI but fails from belrog, confirm the API key is saved in the app.
