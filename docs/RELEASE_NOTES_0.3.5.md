...
- Added automatic lossless source preparation so long local videos can use belrog's faster sequential decoder instead of falling back to repeated random-access seeking.
- Reduced renderer memory pressure during long exports and improved export diagnostics.

## Long-Source Export Performance

- belrog now inspects long local video sources before rendering.
...
- If preparation is unavailable or fails, belrog keeps the existing standard decoder as a fallback instead of failing the export.
