 ...
 * a one-line summary. Import JSON and the Belrog bridge live under Advanced;
...
export default function CustomWorkflowSlotCard({
  ...
  const bridgeMessage = String(
    ...
    || bridgeStatus?.error
    || 'Optional bridge lets ComfyUI send the current graph back to Belrog.'
  ).trim()
    ...
    <div className="mt-3 rounded-lg border border-sf-dark-700 bg-sf-dark-900/70 p-3">
      ...
      {!workflowLoaded && (
        <div className="mt-2 text-[10px] leading-4 text-sf-text-muted">
          ...
          {' '}— it has every Belrog node title already set. Make it yours, then save it to My Workflows.
        </div>
      )}
            ...
            {bridgeInstalled ? (
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="text-[10px] text-emerald-300">
                  Belrog Bridge installed — Send to Belrog is available inside ComfyUI.
                </span>
