...
// custom slot (Belrog node-title contract).
...
function injectMarkerStubNodes(graph, kind, missingKeys) {
  ...
  if (added.length > 0) {
    ...
    graph.groups.push({
      title: 'Belrog: wire these in, then save to My Workflows',
 ...
 * Belrog marker nodes for the given slot injected off to the side of the
...
/** Read a saved workflow's UI-format graph (e.g. for Belrog marker scans). */
