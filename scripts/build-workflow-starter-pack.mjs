...
function renderWorkflowMarkdown(entry) {
  ...
  lines.push('- A ComfyUI-importable copy of the workflow graph bundled with Belrog.')
  ...
  if (entry.requiredModels.length > 0) {
    ...
    lines.push('5. Return to Belrog Generate and click `Re-check` before queueing.')
  } else {
    ...
    if (entry.requiresComfyOrgApiKey) {
      lines.push('4. Add your Comfy account API key in Belrog Settings before queueing.')
      lines.push('5. Return to Belrog Generate and click `Re-check` before queueing.')
    } else {
      lines.push('4. Return to Belrog Generate and click `Re-check` before queueing.')
    }
  }
  lines.push('')
...
function renderReleaseNotes(releaseMetadata) {
  ...
  lines.push('')
  lines.push(`Generated for Belrog v${releaseMetadata.compatibleAppVersion}.`)
  lines.push('')
  ...
  lines.push('- Users who prefer to install nodes and models manually before queueing inside Belrog')
  lines.push('')
  lines.push('## Important Notes')
  lines.push('')
  lines.push('- This pack does not replace the Belrog desktop app.')
