...
2. Name it `belrog-feedback`, deploy the hello-world, then **Edit code** and
...
npx wrangler deploy worker.js --name belrog-feedback --compatibility-date 2026-07-01
npx wrangler secret put DISCORD_WEBHOOK_URL --name belrog-feedback
```

### 3. Point the app at it

The worker gets a URL like `https://belrog-feedback.<account>.workers.dev`.
Either:

- add a custom domain in the worker's settings (e.g. `feedback.belrog.ai`)
...
```js
localStorage.setItem('belrog-feedback-endpoint', 'https://belrog-feedback.<account>.workers.dev')
```

Open Settings > Send Feedback, send a test message, and check the Discord
channel. Remove the override with
`localStorage.removeItem('belrog-feedback-endpoint')`.
